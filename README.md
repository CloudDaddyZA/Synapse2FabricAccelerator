...
- **Offline HTML dashboard** — fully self-contained (inline SVG charts, no CDN), with workspace filtering, drill-down drawers, pipeline-flow diagrams (expandable to full screen), pipeline run analytics, a grouped **Fabric** menu (Fabric Readiness with a **delivery-team & timeline planner**, a **Deploy to Fabric** view that links straight to the FDFMA with per-workspace ARM templates and upload steps, and a **Notebook Modernization** view surfacing the Fabric Notebook Modernizer results), and a grouped **Diagrams** menu (Workspace, Trigger Dependency, Object Dependency, **Dataflow Lineage** — source table → data flow → sink table for the original Synapse data flows — and Lineage views).
...
- **Diagrams** (grouped dropdown) — four dependency visualizations:
  ...
  - **Object Dependency Diagram** — the complete connection map of every workspace object across six columns (triggers → pipelines → notebooks → data flows → datasets → linked services), wiring trigger→pipeline, pipeline→notebook/data-flow/dataset/linked-service, data-flow→dataset/linked-service, and dataset→linked-service edges. Read edges (a pipeline/data flow consuming a dataset) are drawn teal and write edges crimson, so you can see which **tables each object reads and writes**; dataset nodes carry their physical table/path and each object's detail drawer lists its Reads and Writes. A name filter, click-to-focus (isolate an item's dependency chain), per-type toggle chips and a show/hide-unlinked control keep large estates manageable; nodes are capped per column and every node opens its detail drawer.
